...
| Locked dashboard for browser sign-ups | A project with no workflow yet shows a locked dashboard: rail dimmed, one page that sends the person to their coding agent with the install line. Unlocks on the next reload once a workflow exists. `TIN_LITE_BROWSER_LOCK_ENABLED=false` turns it off. | Browser onboarding does not exist yet; the lock stands in for it. |
